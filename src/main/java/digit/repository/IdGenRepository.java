...
import digit.web.models.IdGenReq;
import digit.web.models.IdGenRes;
...
public class IdGenRepository {
     ...
     * @return
     */
    public IdGenRes getId(RequestInfo requestInfo, String tenantId, String name, String format, int count) {

        List<IdRequest> reqList = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            reqList.add(IdRequest.builder().idName(name).format(format).tenantId(tenantId).build());
        }
        IdGenReq req = IdGenReq.builder().idRequests(reqList).requestInfo(requestInfo).build();
        IdGenRes response = restTemplate.postForObject(

...
public class PGRQueryBuilder {
    ...
    private static final String QUERY = "select ser.*,ads.*," + QUERY_ALIAS+
            " from eg_pgr_service ser INNER JOIN eg_pgr_address ads" +
            " ON ads.parentId = ser.id ";

    private static final String COUNT_WRAPPER = "select count(*) from ({INTERNAL_QUERY}) as count";

    private static final String RESOLVED_COMPLAINTS_QUERY = "select count(*) from eg_pgr_service where applicationstatus='CLOSEDAFTERRESOLUTION' and tenantid=? and lastmodifiedtime>? ";

    private static final String AVERAGE_RESOLUTION_TIME_QUERY = "select round(avg(lastmodifiedtime-createdtime)/86400000) from eg_pgr_service where applicationstatus='CLOSEDAFTERRESOLUTION' and tenantid=? ";
